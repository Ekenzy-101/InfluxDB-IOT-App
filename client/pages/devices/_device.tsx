...
}>(() => import("./_devicePlot").then((mod) => mod.DevicePlot), { ssr: false });
...
export function Device({ deviceId }: { deviceId: string | string[] }) {
  ...
  const getMeasurements = (field: string) => {
    const query = `from(bucket: "${process.env.NEXT_PUBLIC_INFLUX_BUCKET}")
         |> range(start: -60d)
         |> filter(fn: (r) => r._measurement == "environment" and r.deviceId == "${deviceId}" )
         |> filter(fn: (r) => r._field == "${field}" )
         |> aggregateWindow(every: 5m, fn: mean, createEmpty: false)
         |> yield(name: "mean")
         `;
    return fetch(`/api/devices/${deviceId}/measurements`, {
      method: "POST",
      headers: {
        Accept: "text/csv",
  ...
  return (
    <div>
      ...
      {device && (
        <p>
          <button onClick={writeSimulatedData}>
      ...
      {data.map((csv, i) => (
        <div className="card-body">
          <div key={`${i}-line`}>
            <DynamicDevicePlotWithNoSSR csv={csv} plot="line" />
          </div>
          <div key={`${i}-table`}>
            <DynamicDevicePlotWithNoSSR csv={csv} plot="table" title="Hello" />
          </div>
        </div>
      ))}
    </div>
